function XPcontourf(arraytoplot,varargin)
% A function to plot the way that I commonly use for XPcorrelate, modified 
% for XPImport. 
% OBLIGATORY INPUTS: 
...
% CMM 2020
try
    defaultx=evalin('base','X');
    defaulty=evalin('base','Y');
...
addOptional(p,'numcol',defnumcol,@isscalar);
...
figure;
hplot=contourf(p.Results.X,p.Results.Y,arraytoplot,p.Results.numcol,'LineColor','None');
